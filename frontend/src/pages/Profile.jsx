...
function Profile() {
  ...
  <div className='bg-white shadow-lg rounded-2xl p-8 max-w-xl w-full relative'>
    <FaArrowLeft className='absolute top-4 left-4 cursor-pointer' onClick={() => navigate("/")} />
  ...
  {userData?.role}
    ...
    <div className='mt-6 space-y-4' >
        ...
        <div className='text-sm text-gray-600 flex items-center justify-start '>
            ...
            <span >{userData?.enrolledCourses.length > 0 ? userData?.enrolledCourses.join(", ") : "No courses enrolled"}</span>
        </div>
    </div>
    <div className='mt-6 flex justify-center gap-4' > 
         <button
  className='px-5 py-2 rounded bg-black text-white active:bg-[#4b4b4b] cursor-pointer transition' onClick={() => navigate("/editprofile")}
>
  Edit Profile
</button>  </div>
  </div>
